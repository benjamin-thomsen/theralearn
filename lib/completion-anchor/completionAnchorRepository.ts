...
export type CompletionAnchorRow = { owner_id: string; package_identity: string; approved_learning_design_identity: string; approved_learning_design_snapshot: string; response_evaluation_contract_identity: string; response_evaluation_contract_snapshot: string; retrieval_interaction_identity: string; terminal_interaction_digest: string; completed_at: string };
export interface CompletionAnchorStore { find(ownerId: string, packageIdentity: string): Promise<CompletionAnchorRow | null>; createOnce(row: Omit<CompletionAnchorRow, "completed_at">): Promise<CompletionAnchorRow>; }
...
function anchorRow(secret: string, ownerId: string, pkg: ApprovedAuthorityPackage, facts: unknown): Omit<CompletionAnchorRow, "completed_at"> {
